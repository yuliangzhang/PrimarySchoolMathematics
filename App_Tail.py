...
import shutil
import random
from Psmrcddup_Tail import Generator
from PrintPreview_Tail import PrintPreview
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.psm_compare_list = [] ## 第二道类型的题目，比较大小题
        self.psm_application_list = [] ## 第三道大题， 应用题
        ...
        self.radio_box_3 = wx.RadioBox(self, wx.ID_ANY, u"题型设置", choices=[u"求结果", u"求算数项", u"比较大小", u"应用题"], majorDimension=1,
                                       style=wx.RA_SPECIFY_ROWS)
    ...
    def producePSM(self, e):
        ...
        if len(self.psm_type) == 0:
            ...
            # 循环生成每套题:定制版 求结果题--10  求算数项--10 比较大小题--10 应用题--2
            templist = []
            for l in self.psm_type:
                print(l)
                g = Generator(l[0], l[1], l[2], l[3], l[4],
                              l[5], l[6], l[7], l[8], l[9], l[10])
                templist = g.generate_data()
                if l[7] <=1:
                    self.psm_list.extend(templist)
                elif l[7] == 2:
                    self.psm_compare_list.extend(templist)
                elif l[7] == 3:
                    self.psm_application_list.extend(templist)

            # for i in range(4):
            #     if i < 3:
            #         g = Generator(l[0], l[1], l[2], l[3], l[4],
            #                   l[5], 10, i, l[8], l[9], l[10])
            #     else:
            #         g = Generator(l[0], l[1], l[2], l[3], l[4],
            #                       l[5], 2, i, l[8], l[9], l[10])
            ...
            pp = PrintPreview([self.psm_list, self.psm_compare_list, self.psm_application_list], self.psm_title,
                              subtit, col=int(self.text_ctrl_3.GetValue()),docxpath=self.config.c.get('config','docx')+os.sep)
            pp.produce_docx()  # 生成docx
            self.psm_list.clear()  # 清空打印列表。
            self.psm_compare_list.clear()
            self.psm_application_list.clear()
            # self.movdocx()
